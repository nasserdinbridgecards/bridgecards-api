...
// ── Trust proxy — required on Render/Railway so rate limiter
...
async function dbInit() {
  if (!MONGODB_URI) {
    log('WARN', 'DB_IN_MEMORY', { msg: 'No MONGODB_URI — data lost on restart.' });
    return;
  }
  try {
...
async function dbSetUser(email, data) {
  if (db?.isMongoose)
    return db.User.findOneAndUpdate({ email: email.toLowerCase() }, data, { upsert: true, new: true }).lean();
...
// ═══════════════════════════════════════════════════════
// LIVE FX RATES  (server-side, refreshed every 60 min)
// ═══════════════════════════════════════════════════════
const FALLBACK_FX = {
...
// ═══════════════════════════════════════════════════════
// PRICING ENGINE  —  gross = (desiredNet + FLAT) / (1 - PCT)
// ═══════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════
// RATE LIMITER
// FIX: app.set('trust proxy',1) above ensures req.ip is
//      the real client IP, not the Render proxy IP.
// FIX: limits raised to prevent false 429s in normal use.
// ═══════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════
// PASSWORD  (PBKDF2 — no external deps)
// ═══════════════════════════════════════════════════════
...
const tpl = (title, body, col = '#3d6bff') =>
...
app.get('/api/health', (req, res) => res.json({
  status: 'ok', sandbox: SANDBOX, version: APP_VERSION,
  commit: APP_COMMIT, requestId: req.requestId,
  db: db?.isMongoose ? 'mongodb' : 'in-memory',
}));

// FIX: no-cache headers ensure frontend always gets the current
//      pk_test or pk_live key from Render env — never a cached stale key
...
// Server-side price — frontend fetches this instead of calculating locally
app.post('/api/price', rateLimit(30, 60000), async (req, res) => {
...
// ═══════════════════════════════════════════════════════
// ORDERS — server recalculates price from Reloadly.
//          Frontend unitPrice is NEVER trusted.
// FIX: limit raised 5→15 to prevent false 429s
// ═══════════════════════════════════════════════════════
app.post('/orders', rateLimit(15, 60000), async (req, res) => {
  const orderId = generateId();
  let order     = null;
  try {
    // Accept many common frontend key variants
    let productId = req.body.productId
      ?? req.body.product_id  ?? req.body.productID
      ?? req.body.product?.productId ?? req.body.product?.id
      ?? req.body.item?.productId    ?? req.body.item?.id
      ?? req.body.cart?.[0]?.productId       ?? req.body.cart?.[0]?.id
      ?? req.body.cartItems?.[0]?.productId  ?? req.body.cartItems?.[0]?.id
      ?? req.body.items?.[0]?.productId      ?? req.body.items?.[0]?.id
      ?? req.body.selectedProduct?.productId ?? req.body.selectedProduct?.id
      ?? null;

    let quantity = req.body.quantity ?? req.body.qty ?? req.body.count
      ?? req.body.product?.quantity  ?? req.body.item?.quantity
      ?? req.body.cart?.[0]?.quantity        ?? req.body.cartItems?.[0]?.quantity
      ?? req.body.items?.[0]?.quantity       ?? req.body.selectedQuantity
      ?? null;

    const paymentIntentId = req.body.paymentIntentId
      ?? req.body.payment_intent_id  ?? req.body.paymentIntentID ?? req.body.intentId
      ...
      ?? (typeof req.body.clientSecret === 'string'
          ? req.body.clientSecret.split('_secret_')[0] : null)
      ?? (typeof req.body.paymentIntentClientSecret === 'string'
          ? req.body.paymentIntentClientSecret.split('_secret_')[0] : null)
      ?? null;

    const { paymentMethod, userEmail } = req.body;
    ...
    if (!customerEmail)
      return res.status(400).json({ error: 'Authentication required', requestId: req.requestId });

    // Recovery: pull productId/qty from Stripe PI metadata if missing
    ...
    if (!productId || !quantity) {
      ...
      return res.status(400).json({
        error: 'Missing fields', missing, requestId: req.requestId,
        hint: 'Send productId & quantity in the body, or include paymentIntentId for metadata fallback.',
      });
    }
    ...
    if (!isNaN(numId)) {
      try {
        ...
        const matched = denoms.find(d => Math.abs(d - frontendDenom) < frontendDenom * 0.1) ?? denoms[0];
        ...
        if (e.message === 'TIMEOUT')
          return res.status(503).json({ error: 'Service unavailable — no charge made. Retry.', orderId, requestId: req.requestId });
        log('WARN','PRICE_CALC_WARN',{ error: e.message });
        return res.status(503).json({ error: 'Live supplier pricing unavailable. Retry in a moment.', orderId, requestId: req.requestId });
      }
    } else {
    ...
    // Validate Stripe payment amount vs server-calculated amount
    ...
    sendEmail({
      to: customerEmail, subject: `✅ Order Confirmed #${orderId} — BridgeCards`,
    ...
    let bestDenom = denoms2[0], bestDiff = Infinity;
    for (const d of denoms2) {
      const sell2 = calcPrice(d * (1 - disc2/100) * r2usd).sell;
      if (Math.abs(sell2 - serverUnitPrice) < bestDiff) { bestDiff = Math.abs(sell2 - serverUnitPrice); bestDenom = d; }
    }

    const rlPayload = {
      productId: numId, countryCode, quantity: order.quantity, unitPrice: bestDenom,
      customIdentifier: orderId, senderName: 'BridgeCards', recipientEmail: customerEmail,
    };
    ...
    if (rlData.errorCode || rlData.status === 'FAILED' || (rlData.message && !rlData.transactions)) {
      ...
      return res.status(400).json({
        orderId, paymentStatus: 'succeeded', orderStatus: 'FAILED',
        error: 'Card purchase failed', refunded: refundDone, requestId: req.requestId,
        refundMessage: refundDone ? '✅ Refund initiated — 3–5 business days' : '⚠️ Contact support@bridgecards.org',
      });
    }
    ...
    res.json({
      ...rlData, orderId, invoiceNo: order.invoiceNo, requestId: req.requestId,
      paymentStatus: 'succeeded', orderStatus: 'SUCCESSFUL', status: 'SUCCESSFUL',
    });
...
app.get('/orders/user/:id', auth, async (req, res) => {
  ...
  if (o.userId !== req.user.userId && o.email !== req.user.email)
...
app.post('/api/stripe-webhook', async (req, res) => {
  ...
  try {
    ...
    if (v1 !== expected) { log('WARN','WEBHOOK_SIG_INVALID'); return res.status(400).json({ error: 'Invalid signature' }); }
    ...
    if (event.type === 'charge.dispute.created') {
      ...
      sendEmail({ to: ADMIN_EMAIL, subject: `⚠️ CHARGEBACK: ${d.charge}`,
        html: tpl('⚠️ Chargeback', `
          <div class="r"><span>Charge</span><span>${d.charge}</span></div>
...
app.get('/admin/stats', admin, async (req, res) => {
  ...
  res.json({
    ...
    version: APP_VERSION, commit: APP_COMMIT, sandbox: SANDBOX,
    fraudAlerts:  fraudLog.slice(-20),
    recentOrders: orders.slice(0,10).map(o => ({
      id: o.id, email: o.email, status: o.status, amount: o.totalAmount, createdAt: o.createdAt,
    })),
  });
});
...
// FIX: limit raised 5→10
app.post('/api/check-product', rateLimit(10, 60000), async (req, res) => {
  try {
...
// FIX: limit raised 5→15
app.post('/api/create-payment-intent', rateLimit(15, 60000), async (req, res) => {
  try {
    ...
    const productId = req.body.productId
      ?? req.body.product_id ?? req.body.productID ?? req.body.selectedProductId
      ?? req.body.product?.productId ?? req.body.item?.productId
      ?? req.body.cart?.[0]?.productId ?? req.body.items?.[0]?.productId
      ?? req.body.selectedProduct?.productId ?? '';

    const quantity = req.body.quantity ?? req.body.qty ?? req.body.count
      ?? req.body.product?.quantity ?? req.body.cart?.[0]?.quantity
      ?? req.body.items?.[0]?.quantity ?? req.body.selectedQuantity ?? 1;
...
app.listen(PORT, async () => {
  ...
  await seedAdmin();
  log('INFO','STARTED',{
    port: PORT, sandbox: SANDBOX, version: APP_VERSION,
    db: db?.isMongoose ? 'mongodb' : 'in-memory',
  });
});
